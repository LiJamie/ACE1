...
import edu.ntut.selab.equivalent.CompositeLV01OR;
...
public class AndroidCrawlerTest {
    ...
    public void setup() throws InterruptedException, ExecuteCommandErrorException, ClickTypeErrorException, DocumentException, IOException, NullPackageNameException, MultipleListOrGridException {
        ...
        this.stateGraph.setEquivalentStragegy(new CompositeLV01OR());
    ...
    public void testFirstTimeRestartApp() throws InterruptedException, ExecuteCommandErrorException, IOException, NoSuchFieldException, IllegalAccessException, ClickTypeErrorException, DocumentException, NullPackageNameException, MultipleListOrGridException, EquivalentStateException, ProgressBarTimeoutException {
        ...
        String[] getPackageNameCommand = {"adb", "-s", this.deviceSerialNum, "shell", "dumpsys", "window", "windows", "|", "grep", "-E", "\'mFocusedApp\'"};
        ...
        if (!Config.APP_INSTRUMENTED) {
            String[] getActivityNameCommand = {"adb", "-s", this.deviceSerialNum, "shell", "dumpsys", "window", "windows", "|", "grep", "-E", "\'mCurrentFocus\'"};
            List<String> activityNameResult = CommandHelper.executeCmd(getActivityNameCommand);
            assertTrue(activityNameResult.get(0).contains(Config.LAUNCHABLE_ACTIVITY));
        }
    ...
    @Test
    public void testGetNondeterministicEventIfExist() throws DocumentException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        GUIState nonDeterministicState = new GUIState(DocumentHelper.parseText("<text></text>"), new ArrayList<AndroidEvent>());
        backKeyEvent.setToState(nonDeterministicState);
        Method method = AndroidCrawler.class.getDeclaredMethod("getNondeterministicEventIfExist", GUIState.class, AndroidEvent.class);
        method.setAccessible(true);
        AndroidEvent nonDeterministicEvent = (AndroidEvent) method.invoke(this.androidCrawler, nonDeterministicState, clickEvent);
