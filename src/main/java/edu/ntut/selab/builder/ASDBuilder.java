...
public class ASDBuilder extends ReportBuilder {
    ...
    /*
     * index = activityList�̭��ؼ�activity����m
     */
                    ...
                    if (e.getToState() != null) { // Problem : somewhere e.getToState is null
                        ...
                        if (e.getToState().getActivityName().equals(super.getActivityNames(this.stateGraph).get(index))) {
                            if (e.getToState().getId() == s.getId()) { // self loop
                                ...
                                if (multipleTransitionAggregation) {
                                    if (e.isNonDeterministic()) {
                                        if (nondeterministicMultiTransitionMap.get(e.getToState().getId()) != null) {
                                            str = nondeterministicMultiTransitionMap.get(e.getToState().getId());
                                        }
                                        str = str + this.displayOrder(e) + "\n";
                                        nondeterministicMultiTransitionMap.put(e.getToState().getId(), str);
                                    } else {
                                        if (multiTransitionMap.get(e.getToState().getId()) != null) {
                                            str = multiTransitionMap.get(e.getToState().getId());
                                        }
                                        str = str + this.displayOrder(e) + "\n";
                                        multiTransitionMap.put(e.getToState().getId(), str);
                                    }
                                } else { // no aggregation
                                    if (e.isNonDeterministic()) {
                                        writer.print("state_" + s.getId() + " -> ");
    ...
    private void createSubGraph(GUIState s) throws InterruptedException, ExecuteCommandErrorException, IOException {
        ...
        this.createStateNodes(writer, s);
        writer.println("}");
        writer.close();
        createSVGFile(dotFile, targetSVGFile);
    }

    private void createStateNodes(PrintWriter writer, GUIState s) {
        int counter = 1;
        String str = "";
